...
### 전망에 값을 더한 판을 올린 뒤 — 최대 12시간은 옛 답이 나온다

전망 캐시(`us-outlook`·`kis-outlook`)는 **12시간**이고 **판 번호를 안 매긴다**(담기는 것이 상대가
준 값이다 — `CacheNames`의 규칙). 그래서 배당처럼 **필드를 더한 판**을 올리면 옛 항목이 그 칸을
비운 채로 읽혀 그 사이 새 줄이 안 나온다. 기다리면 스스로 낫는다.

바로 보고 싶으면 그 캐시만 비운다.

```bash
curl -X DELETE "$HOST/actuator/caches/us-outlook"
curl -X DELETE "$HOST/actuator/caches/kis-outlook"
```

⚠️ **`/actuator/evict`가 아니다.** 그쪽은 `name`과 `key`를 **둘 다** 요구하고, 하나만 주면
`{"evicted":false}`를 돌려준다 — 통째로 비우는 것은 Boot의 `/actuator/caches/{이름}`이다.
(처음에 `evict`를 적어 뒀는데, 그 런북을 따르면 두 번 `false`를 받고 「캐시가 안 비워진다」고
결론 내게 된다.)

⚠️ **비우면 다음 조회가 한도를 다시 쓴다** — 미국은 심볼당 FMP 3회, 국내는 종목당 KIS 간격 2초
(투자의견·배당일정)다. **ETF는 1초**다 — 투자의견을 안 묻는다.
