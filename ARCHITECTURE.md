...
- **캐시 히트가 리미터 퍼밋을 태운다.** KIS는 초당 1건이라 브리핑의 KIS 조회 9회가 전부
  캐시에 있어도 ~8초를 기다린다 — 캐시가 아무것도 아껴 주지 않는 상태다.
...
`RenderedOutputTest`가 57가지를 렌더해 `golden/messages.txt`와 대조한다. 손으로 돌리던 방식
(이전 커밋으로 되돌려 찍고 `diff`)은 커밋 사이에 오라클이 사라져 "커밋마다 초록"을 보장하지 못했다.

들여온 값이 즉시 나왔다 — 단위 테스트 448개가 전부 초록인 채로 살아 있던 버그 셋이 골든을
...
- **중복 발송이 완전히 막히지는 않는다.** 비영속 저장소(Render Key Value 등)가 오후에 비면
  ...
  크론이 아예 안 돈다.
- **KIS 앱키가 없으면 브리핑이 헛호출 9번을 리미터에 태운다.** 초당 1건이라 그만큼 늦어진다.
  발송 창이 두 시간이라 문제는 안 되지만 공짜는 아니다.
- **한 무리에 실시간과 종가가 섞이면 어느 값이 어느 성격인지 값 줄만 보고는 모른다.**
  꼬리에 기준을 두 줄 쌓아 밝히되 값마다 꼬리표를 달지는 않는다 — 그러면 값 줄이 지저분해진다.
  의도한 맞바꿈이다.
- **단위 테스트가 통과해도 이어 붙이면 틀릴 수 있다.** 448개가 전부 초록인 상태에서
...
- **NYSE 종목은 지금 어느 출처로도 못 온다.** KIS 모의 계정의 `EXCD=NYS` 조회가 HTTP 500이고
  (2026-08-19 실측: PATH·ORCL·SNOW) 2순위 FMP 무료 티어가 그 심볼들을 402로 막는다.
  NAS는 같은 코드로 잘 돌아가므로 차이가 전부 상대 쪽이다 — 코드로 고칠 수 있는 것이 아니다.
  화면은 틀린 값을 만들지 않고 "찾지 못했습니다"로 답한다(`CLAUDE.md`에 자세히 적어 뒀다).
