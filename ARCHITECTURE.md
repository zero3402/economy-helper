...
- **성공하면 즉시 반환한다.** 매번 전부 불러 가장 신선한 것을 고르는 방식은 이중화가 아니라
  *선택*이고, 요청마다 2순위의 한도(수출입은행 하루 1,000회, FMP 하루 250회 — 그중 전망이
  심볼당 3회를 쓴다)를 태운다.
...
- ⚠️ **「보충」은 폴백이 아니다 — 실패해도 답을 죽이지 않고 그 줄만 빠진다.** 전망(목표가·
  실적발표일·배당) · 일봉 차트 · 강수 시각이 그렇다. 반대로 **사용자가 물은 날·값 자체를 내는
...
| 사진이 글보다 늦게 붙던 것 | 그 `Supplier`를 **글을 보낸 뒤에** 불러서, 조회 시간이 발송 간격(같은 방에 초당 한 통) **위에 그대로 얹혔다** — 실측(2026-09-08) 글과 사진 사이가 하한 1.0초에서 **2.4초**까지 벌어졌다 | 답이 다 만들어진 뒤 **가상 스레드에 띄워** 글 발송과 겹친다. 순서는 그대로 글 → 사진이고, 답의 KIS 호출들이 간격 문에 먼저 줄 서 있으므로 **글은 안 늦고 사진만 빨라진다**. 예전 모양(답을 만들 때 함께 받기)으로 되돌리는 것이 아니다 |
