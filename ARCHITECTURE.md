...
- **바이낸스는 리전에 묶여 있었다.** 미국 IP에서 451이 떨어지고, 4xx라 브레이커도 안 열려
  매 호출이 나가 매번 실패했다 — Render 미국 리전에서 실제로 그 상태였다. 지금은 공개 데이터
  미러(`data-api.binance.vision`)로 **우회**한다. 이중화가 아니라 우회다: 상대가 죽어서가 아니라
  우리 IP가 막혀서 가는 길이라, 1순위가 살아 있으면 2순위는 안 불린다.
  ⚠️ **미러까지 막히는 리전이라면 그때는 정말로 리전을 옮기는 수밖에 없다.**
