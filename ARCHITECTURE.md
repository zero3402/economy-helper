...
| **실전 계정 (KIS)** | 실전 전용 엔드포인트가 열린다 — `invest-opinion`(종목투자의견)·`inquire-index-price`·종목명 검색 | 모의는 계좌 없이 붙일 수 있지만 그 셋이 막힌다 | 도메인과 앱키가 **짝**이다. 실전 키로 모의를 부르면 토큰 발급이 403(`EGW00105`)이고 **KIS가 통째로 죽는다** — 실측 2026-08-20에 실제로 그 상태였다 |
