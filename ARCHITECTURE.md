...
3. **스프링을 띄우지 않는다.** WireMock + `new`로 만든다. 컨텍스트가 필요한 것은
   ...
   **거기서 스케줄 잡이 함께 산다.**
   - ⚠️ **WireMock 서버는 클래스당 하나다.** `@BeforeAll`에서 한 번 띄우고 테스트 사이에는
     `@BeforeEach`의 `server.resetAll()`로 **상태만** 되돌린다 — 스텁·요청기록·시나리오가
     함께 비므로 `verify(n, …)` 같은 횟수 단언도 그대로 성립한다(`RetryLiveTest`가 그 위에서
     정확한 횟수와 시나리오를 함께 쓴다). **테스트마다 띄우고 내리면 포트 재활용 창이 열리고**,
     그 창에서 요청이 앞 테스트의 서버에 닿는 것을 §7에서 실측했다.
     `WireMockLifecycleTest`가 `@BeforeEach`가 서버를 띄우지 않는지 훑어 그 규칙을 지킨다.
...
- ⚠️ **전체 실행에서 WireMock 클라이언트 테스트가 드물게 떨어졌다 — 원인은 아직 미확정이지만
  ...
  - **그래서 원인을 증명하는 대신 그 모양을 없앴다.** WireMock을 쓰는 25개 클래스 전부가
    이제 서버를 `@BeforeAll`에서 한 번 띄운다(§6의 그 규칙). 포트 재활용 창이 사라졌으므로
    가설이 맞았든 틀렸든 **그 경로로는 못 깨진다.**
