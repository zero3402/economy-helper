...
support/    Concurrently (가상 스레드 팬아웃)
```
...
**출력은 읽어서가 아니라 찍어 봐야 드러난다.** 알람·검색·안내 49가지를 전부 렌더해 눈으로
훑는 방식으로 버그 둘을 찾았다(온도 자릿수, 도움말의 `/stock` 표기). 리팩터링이
화면을 건드리지 않았다는 증명도 같은 방식이다 — 이전 커밋으로 되돌려 같은 하네스로 찍고 `diff`한다.
