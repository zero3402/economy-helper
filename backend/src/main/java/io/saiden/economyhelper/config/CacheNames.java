...
public final class CacheNames {
     ...
     *
     * <p><b>판은 안 매긴다</b> — 담기는 것이 상대가 준 값이다. {@link #US_OUTLOOK}과 마찬가지로
     * 배당이 붙던 날 옛 항목은 {@code dividend}가 {@code null}로 읽혀 최대 12시간 그 블록이 안 나오고
     * 스스로 낫는다.
     */
    public static final String KIS_OUTLOOK = "kis-outlook";

    /**
     * 미국 종목의 <b>목표주가·실적발표일·배당</b>. <b>심볼당 FMP 호출이 셋</b>
     * ({@code price-target-consensus}·{@code earnings}·{@code dividends}가 다른 엔드포인트이고
     * 무료 티어는 배치가 막혔다)이라 하루 250회에서 심볼 하나가 3회를 쓴다 — 이 캐시가 그 한도의
     * 실질 방어다.
     *
     * <p>⚠️ 한동안 「목표주가·<b>투자의견</b>」이었고 둘째 호출도 「의견」이라 적혀 있었다 —
     * 투자의견을 걷어내면서 {@code grades-consensus}를 지우고 실적발표일이 그 자리에 왔는데
     * 이름만 남았다. 그때 개수(둘)는 우연히 그대로였고, 배당이 붙어 셋이 됐다.
     *
     * <p><b>판은 안 매긴다</b> — 담기는 것이 상대가 준 값이다. 배당이 붙던 날 옛 항목은
     * {@code dividend}가 {@code null}로 읽혀 최대 12시간 그 블록이 안 나오고 스스로 낫는다.
     */
