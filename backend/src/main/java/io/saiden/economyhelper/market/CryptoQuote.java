 ...
 *
 * <p><b>등락률을 담는다.</b> 한동안 뺐고 그 근거는 "토스가 등락률을 주지 않아 {@code /stock}에
 * 넣을 수 없으니 코인에만 붙이면 정보 밀도가 달라진다"였다. <b>그 전제가 낡았다</b> —
 * 주가 출처가 토스에서 FMP와 공공데이터포털로 바뀌었고 넷 다 등락률을 준다. 이제는
 * 환율·주식·코인이 모두 같은 밀도라 오히려 넣는 쪽이 일관된다.
 *
