 ...
 * 막지 못한다. 이 클래스는 <b>실행 시각이 아니라 예정된 슬롯</b>으로 키를 잡으므로 시계가
 * 얼마나 밀렸든 "8월 11일 아침 발송"은 한 번뿐이다. 수동 트리거에도 같은 보호가 걸린다.
 *
 * <p><b>슬롯 이름을 만드는 일은 여기가 아니라 {@link DigestSlot}이 한다.</b> 저장소가 하나라
 * 잡이 여럿이면 이름이 겹칠 수 있는데, 실제로 한 번 겹쳐 브리핑이 통째로 사라진 적이 있다.
 *
