 ...
 *
 * <p><b>줄임말도 여기 있다.</b> {@code /c}·{@code /f}·{@code /s}·{@code /h}·{@code /w}는
 * 파서가 아니라 상수가 든다 — 그래야 {@link HelpFormatter}가 열거형을 훑는 것만으로
 * 도움말에 함께 따라 나온다. 목록을 파서에 두면 도움말이 그것을 모르는 <b>두 번째 표</b>가
 * 되고, 이 파일이 이미 한 번 그 함정으로 어긋난 적이 있다(「주식」 대 「증시」).
 */
public enum Command {
     ...
     *
     * <p>줄임말이 없다. {@code /n}을 붙일 자리는 비어 있지만 요청에 없던 것을 지어내면
     * 나중에 진짜로 필요한 명령이 그 글자를 못 쓴다.
     */
    NEWS("/news", null, "뉴스", true, "/news 금리"),
