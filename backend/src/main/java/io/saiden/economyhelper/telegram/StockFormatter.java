...
public final class StockFormatter {
     ...
     *
     * <p><b>값의 성격도 이름표가 든다</b>({@code 실적발표일(미국)}). 이 통은 <b>값 줄에
    ...
    /**
     * 전망까지 붙인 것 — 목표주가·실적발표일이 있는 종목에만 줄이 는다.
     *
     ...
     * 무리 제목의 「미국」이 위에 있긴 하지만 그 사이에 이름·값·환산·등락률·목표가 블록이
     * 끼어 있어, 이 줄에 닿을 때쯤엔 그 머리와 연결이 끊긴다. 그렇다고 {@code (미국 현지)}로
     * 적으면 <b>나라를 적은 뒤에도 「현지」가 남아</b> 넘친다 — 그 낱말이 더 말해 주는 것이
     * 없다. 가운데가 {@code (미국)}이다. 괄호 앞 공백도 두지 않는다: 이름표는 한 낱말로 읽힌다.
     *
